...
.pj-big-arrow.left { left: -70px; }
.pj-big-arrow.right { right: -70px; }
...
.pj-ghost {
  font-family: 'Outfit', sans-serif;
  font-size: clamp(40px, 5vw, 70px); font-weight: 900;
  line-height: 0.85; letter-spacing: -0.05em; color: transparent;
...
.pj-title {
  font-family: 'Outfit', sans-serif;
  font-size: clamp(24px, 2.5vw, 38px); font-weight: 900;
  text-transform: uppercase; letter-spacing: -0.01em;
  line-height: 1; color: var(--text-primary, #f8fafc);
  margin-bottom: 8px;
}

.pj-rule { height: 2px; width: 36px; border-radius: 2px; margin-bottom: 12px; }

.pj-desc {
  font-family: 'Outfit', sans-serif; font-size: 13px;
  font-weight: 300; color: var(--text-sub, #cbd5e1);
  line-height: 1.5; margin-bottom: 16px;
}
...
.pj-btn-gh {
  ...
  border: 1px solid var(--card-border, rgba(255,255,255,0.15)); background: transparent;
  color: var(--text-primary, #f8fafc); cursor: pointer; text-decoration: none;
  display: inline-flex; align-items: center; gap: 6px;
  transition: background 0.15s ease, transform 0.15s ease;
}
.pj-btn-gh:hover { background: rgba(255, 255, 255, 0.08); transform: translateY(-1px); }
...
.pj-dot.active { transform: scale(1.4); }
...
.pj-lightbox-inner { position: relative; display: inline-flex; }
.pj-lightbox-close {
  position: absolute; top: -16px; right: -16px;
  width: 36px; height: 36px; border-radius: 50%;
  background: #ffffff; border: none; color: #0f172a;
  font-size: 14px; font-weight: bold; cursor: pointer;
}

@media (max-width: 1024px) {
  .pj-big-arrow.left { left: 10px; }
  .pj-big-arrow.right { right: 10px; }
  .pj-section { padding: 40px 10px; }
}
...
@media (max-width: 900px) {
  ...
  .pj-img-card { height: 280px; }
  .pj-big-arrow { display: none; }
}
`;
...
function Projects() {
    ...
    // Auto change project timer (pauses on hover)
    useEffect(() => {
    ...
    const goTo = (idx) => {
        setCurrent(Math.min(projects.length - 1, Math.max(0, idx)));
    };
    ...
    return (
        ...
        >
            ...
            <div className="pj-outer-wrapper">
                <button
                    className="pj-big-arrow left"
                                ...
                                <div className="pj-dots">
                                    {projects.map((_, i) => (
                                        <div
                                            key={i}
                                            className={`pj-dot ${i === current ? 'active' : ''}`}
                                            style={{ background: i === current ? project.accent : undefined }}
                                            onClick={() => goTo(i)}
                                        />
                                    ))}
                                </div>
