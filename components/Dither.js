...
export default function Dither({
    ...
    const checkMobile = useCallback(() => {
        ...
        const lowPower = window.innerWidth <= 768 || coarse || reduceMotion;
        setIsMobile(lowPower);
        setUsePostProcessing(!reduceMotion && !coarse && window.innerWidth > 768);
    }, []);
    ...
    if (!isVisible || isMobile) {
        return (
            <div
                ...
                style={{
                    background:
                        'radial-gradient(circle at top, rgba(74, 110, 255, 0.28), transparent 45%), radial-gradient(circle at bottom, rgba(112, 40, 255, 0.2), transparent 60%)',
                }}
            />
        );
    }
    ...
    return (
        <div ref={wrapperRef} className="w-full h-full relative" style={{ background: 'radial-gradient(circle at center, rgba(72, 99, 255, 0.22), rgba(10, 11, 18, 0.88) 70%)' }}>
            <Canvas
                className="w-full h-full relative"
