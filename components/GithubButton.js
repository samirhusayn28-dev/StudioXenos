...
const GithubIcon = memo(function GithubIcon({ style }) {
  return (
    <svg
      style={style}
      viewBox="0 0 496 512"
...
function GithubButton({ href = '#' }) {
  ...
  const active = !isTouchDevice && hovered;

  const btnStyle = useMemo(
    () => (active ? { ...styles.btn, ...styles.btnHover } : styles.btn),
    [active]
  );

  const svgStyle = useMemo(
    () => ({ ...styles.svgIcon, opacity: active ? 0 : 1 }),
    [active]
  );

  const textStyle = useMemo(
    () => ({ ...styles.text, opacity: active ? 1 : 0 }),
    [active]
  );
